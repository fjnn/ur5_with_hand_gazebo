...
import rospy
import actionlib
from control_msgs.msg import * as cm
from trajectory_msgs.msg import * as tm
...
init = [0.0, -1.57, 1.57, 3.1415, 0.0, 0.0]
...
def move_init():
    ...
    try:
        ...
        g.trajectory.points = [
            JointTrajectoryPoint(positions=joints_pos, velocities=[0]*6, time_from_start=rospy.Duration(0.0)),
            JointTrajectoryPoint(positions=init, velocities=[0]*6, time_from_start=rospy.Duration(2.0))]
        client.send_goal(g)
        client.wait_for_result()
    except KeyboardInterrupt:
        client.cancel_goal()
...
def point_head_to(position, frame, pointing_frame=None, pointing_axis=None):
...
def main():
    ...
    try:
        ...
        if (inp == 'y'):
            move_init()
        inp = raw_input("Continue HEAD? y/n: ")[0]
        if (inp == 'y'):
						point_head_to(self, position, frame, pointing_frame=None, pointing_axis=None):
            #move_repeated()
            #move_disordered()
            #move_interrupt()
            if
